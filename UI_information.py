...
class ImageViewer_1(QtWidgets.QMainWindow, Public):
    def __init__(self, video_name):
        super(ImageViewer_1, self).__init__()
    ...
    def init_ui(self):
        ...
        # 二
        self.title = QtWidgets.QPushButton(qtawesome.icon('fa.car', color='white'), " 基础信息")
        ...
        self.left_layout.addWidget(self.pushButton_2, 575, 28)
    ...
    def image(self):
        ...
        self.number = re.findall(r"(\d+)_", imagefile)
        self.number = str(self.number)
        self.name = 'id' + self.number
        self.name = ''.join(list(filter(str.isalnum, self.name)))
        self.name = '.\\' + self.video_name + '\\' + self.name + '.txt'
        print(self.name)
    ...
    def viewlist(self):
        ...
        self.list.sort(key=lambda x: (x[6]))
        print(self.list)
        for i in self.list:
            if i.endswith('png'):
