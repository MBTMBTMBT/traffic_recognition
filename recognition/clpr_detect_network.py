...
class Network(object):
    ...
    def save_parameters(self, path='D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\mats\\network_parameters.npy'):
        np.save(path, self.parameters)

    def load_parameters(self, path='D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\mats\\network_parameters.npy'):
    ...
    def multilayer_forward(self, x_mat):
        ...
        a_mat_of_l_layers, cache = Network.linear_activation_forward(a_mat, self.parameters['W' + str(length)],
                                                                     self.parameters['b' + str(length)], 'sigmoid')
        self.caches.append(cache)
    ...
    def multilayer_backward(self, AL, Y):
        ...
        self.grads["dA" + str(L - 1)], self.grads["dW" + str(L)], self.grads["db" + str(L)] \
            = Network.linear_activation_backward(dAL, current_cache, 'sigmoid')
    ...
    @staticmethod
    def compute_cost(a_mat_of_l_layers, y_vec):
        m = y_vec.shape[1]
        cost = - 1 / m * np.sum(y_vec * np.log(a_mat_of_l_layers) + (1 - y_vec) * np.log(1 - a_mat_of_l_layers))
        '''
        if cost >= 1526592.390807:
            print(a_mat_of_l_layers)
            print(np.log(a_mat_of_l_layers))
            print(y_vec)
            exit(1)
        '''
    ...
    def load_data_sets(labels: [], path: str, size: ()):
        ...
        for label in labels:
            ...
            for pic in pics:
                if pic.split('.')[1] == 'png' or pic.split('.')[1] == 'jpg':
                    ...
                    # cv2.waitKey(1)
                    # 先按3通道来了
                    pic = np.reshape(pic, (pic.shape[0] * pic.shape[1] * pic.shape[2], 1))
                    '''
                    if len(pic.shape) == 3:
                        pic = np.reshape(pic, (pic.shape[0] * pic.shape[1] * pic.shape[2], 1))
                        # pic = pic.reshape(pic.shape[0], -1).T
                    else:
                        pic = np.reshape(pic, (pic.shape[0] * pic.shape[1] * pic.shape[2] * pic.shape[3], 1))
                        # pic = pic.reshape(pic.shape[0], -1).T
                    '''
                    # pic = pic.reshape(pic.shape[0], -1).T
                    # print(pic)
                    pic_list.append(pic)
        pic_arr = np.zeros((size[0] * size[1] * 3, len(pic_list)), dtype='int32')
        ...
        label_arr = np.zeros((1, label_num))
        for j in range(label_arr.shape[1]):
            label_arr[0][j] = int(label_list[j])
...
def train_test(path: str, save_path='D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\mats\\network_parameters.npy',
               labels=None, size=(108, 40)):
    if labels is None:
        labels = ['1', '0']
    pic_arr, label_arr = Network.load_data_sets(labels, path, size)
    # print(pic_arr)
    network = Network((pic_arr.shape[0], 100, 100, 1))
    network.initialize_parameters()
    network.save_parameters(save_path)
    network.train(pic_arr, label_arr, print_cost=True, num_iterations=20000, learning_rate=0.005)
    network.save_parameters(save_path)


def predict_test(path: str, load_path='D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\mats\\network_parameters.npy',
                 labels=None, size=(108, 40)):
    if labels is None:
        labels = ['1', '0']
    pic_arr, label_arr = Network.load_data_sets(labels, path, size)
    # print(pic_arr)
    network = Network((pic_arr.shape[0], 100, 10, 5, 1))
    # network.initialize_parameters()
    ...
    mistake /= label_arr.shape[0] * label_arr.shape[1]
    correct = 1 - mistake
    print("correct rate: %.2f" % correct)


if __name__ == '__main__':
    # train_test('D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\plates\\train',
    #            save_path='D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\mats\\clpr_plate_distinguishing.npy')
    predict_test('D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\plates\\train',
                 load_path='D:\\MyFiles\\PROGRAMS\\python\\TryTryTry_continue\\mats\\clpr_plate_distinguishing.npy')
